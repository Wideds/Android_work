        ...
        if(requestId == 0){
            ...
            try {
                ...
                for(int i=0; i<arr.length(); i++){
                    JSONObject tmp=arr.getJSONObject(i);//JSONObject에는 num,writer,caption,imagePath가 들어있다.
                    ...
                    String url="http://192.168.0.23:9000/boot07/gallery/images/"+tmp.getString("imagePath");
                    dto.setImagePath(url);
                    //GalleryDto 를 List 에 누적 시킨다.
                    list.add(dto);
                }
                //모델이 업데이트 되었다고 어텝터에 알려서 ListView 가 업데이트 되도록한다.
